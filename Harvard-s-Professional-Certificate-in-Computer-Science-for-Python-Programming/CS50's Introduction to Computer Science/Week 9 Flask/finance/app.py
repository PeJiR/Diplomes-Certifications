...
def buy():
    ...
    if request.method == "GET":
        ...
        transaction_value = shares * int(stock["price"])
        ...
        if user_cash < transaction_value:
            return apology("Sorry but not enough money")

        upt_cash = (user_cash) - transaction_value
...
def money():
    ...
    if request.method == "GET":
        ...
        if not new_cash:
            return apology("You mush add some money honey")
        user_id = session["user_id"]

        user_cash_db = db.execute("SELECT cash FROM users WHERE id = :id", id=user_id)
        user_cash = int(user_cash_db[0]["cash"])

        upt_cash = (user_cash) + new_cash
...
def sell():
    ...
    if request.method == "GET":
        ...
        user_shares_current = user_shares[0]["shares"]

        if shares > user_shares_current:
            return apology(" you do not have enough shares mate! go and get some shares before")
        ...
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, date) VALUES (?, ?, ?, ?, ?)",
                   user_id, symbol, (-1)*shares, stock["price"], date)

        flash(" You sold it mate")
        return redirect("/")
